...
function ListBrowseProducts({row}) {
   ...
   const handleSubmit = async(data) => {
    dispatch(
        browsProduct({ dataOrthers: data?._id}, enqueueSnackbar)
    );
   }
  return (
    <TableRow key={row._id} sx={{ height: "100px" }}>
      ...
      <StyledTableCellBody>
        {!isDisabled && (<IconButton onClick={()=> handleSubmit(row)}><OfflinePinIcon style={{ color:'rgb(46, 125, 50)',fontSize:"25px"  }}/></IconButton>)}
      </StyledTableCellBody>
    </TableRow>
  )
}
